...
exports.ExcelImporter = exports.IMPORT_FIELDS = void 0;
exports.normalizePlatform = normalizePlatform;
exports.normalizeTestType = normalizeTestType;
...
function normalizePlatform(value) {
    ...
    if (v.includes('cms'))
        return 'CMS';
    if (v.includes('web') || v.includes('website'))
        return 'Web';
    if (v.includes('app') || v.includes('mobile') || v.includes('di dong') || v.includes('android') || v.includes('ios'))
        return 'App';
    if (v.includes('api'))
        return 'API';
    return 'App';
...
function normalizeTestType(value) {
    ...
    if (!v)
        return 'Luồng chuẩn';
    if (v.includes('ngoai le') || v.includes('negative') || v.includes('error') || v.includes('exception') || v.includes('sai') || v.includes('loi'))
        return 'Luồng ngoại lệ';
    if (v.includes('bien') || v.includes('boundary') || v.includes('edge') || v.includes('ranh'))
        return 'Giá trị biên';
    if (v.includes('chuan') || v.includes('normal') || v.includes('happy') || v.includes('positive') || v.includes('dung'))
        return 'Luồng chuẩn';
    return 'Luồng chuẩn';
}
function normalizePriority(value) {
    ...
    if (!v)
        return 'Cao';
    if (v.includes('cao') || v.includes('high'))
        return 'Cao';
    if (v.includes('trung binh') || v.includes('medium') || v.includes('tb'))
        return 'Trung bình';
    if (v.includes('thap') || v.includes('low'))
        return 'Thấp';
    return 'Cao';
}
function suggestMapping(headers) {
    ...
    for (const field of exports.IMPORT_FIELDS) {
        ...
        if (!match) {
            match = normalizedHeaders.find((h) => aliases.some((a) => h.norm.includes(a) || a.includes(h.norm)));
        }
        if (match) {
...
function readRowObject(worksheet, rowNumber, headers) {
    const obj = {};
...
class ExcelImporter {
    ...
    static async parse(buffer, mapping) {
        const workbook = new exceljs_1.default.Workbook();
        await workbook.xlsx.load(buffer);
        const worksheet = workbook.worksheets[0];
        if (!worksheet) {
        ...
        for (let r = 2; r <= totalRows; r++) {
            ...
            const allEmpty = exports.IMPORT_FIELDS.every((f) => !get(f.key));
            if (allEmpty)
                continue;
            if (!title) {
