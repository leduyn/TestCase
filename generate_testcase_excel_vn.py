...
def create_excel():
    ...
    # ── Header cột ────────────────────────────────────────────────────────────
    headers = [
        "Test Case ID", "Chức năng (Module)", "Tiêu đề (Mô tả)", "Loại kiểm thử",
            ...
            for col_idx, val in enumerate(values, start=1):
                ...
                if col_idx == 1:  # ID
                    ...
                    cell.font      = Font(name="Calibri", bold=True, size=9,
                                          color=("276221" if tc["type"]=="Luồng chuẩn"
                                                 else "7B2C00" if tc["type"]=="Luồng ngoại lệ"
                                                 else "7B6300"))
                    cell.alignment = align_center
    ...
    summary_headers = ["Nhóm", "Từ TC", "Đến TC", "Số TC", "Luồng chuẩn", "Luồng ngoại lệ", "Giá trị biên"]
    summary_data = [
    ...
    # ── Lưu file ───────────────────────────────────────────────────────────────
    output_path = r"d:\Java lean\TestCase\TestCase_VINAGO_KhachHang_TiengViet.xlsx"
    wb.save(output_path)
